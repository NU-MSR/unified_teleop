...
int main(int argc, char * argv[])
{
    ...
    auto node = rclcpp::Node::make_shared("delta_direct_joystick");
    ...
    RCLCPP_INFO(rclcpp::get_logger("delta_direct_joystick"), ("Currently using the " + device_name + " input device").c_str());
...
static InputType input_type(const std::string input_name)
{
    if (input_name.empty())
    {
        RCLCPP_ERROR(rclcpp::get_logger("delta_direct_joystick"), "Provided input is empty");
        rclcpp::shutdown();
    ...
    switch (first_character)
    {
        ...
        default:
            RCLCPP_ERROR(rclcpp::get_logger("delta_direct_joystick"), "Unable to determine input type");
            rclcpp::shutdown();
