...
int main(int argc, char * argv[])
{
    ...
    RCLCPP_INFO(rclcpp::get_logger("mobile_joystick"), ("Currently using the " + device_name + " input device").c_str());
...
static InputType input_type(const std::string input_name)
{
    if (input_name.empty())
    {
        RCLCPP_ERROR(rclcpp::get_logger("mobile_joystick"), "Provided input is empty");
        rclcpp::shutdown();
    ...
    switch (first_character)
    {
        ...
        default:
            RCLCPP_ERROR(rclcpp::get_logger("mobile_joystick"), "Unable to determine input type");
            rclcpp::shutdown();
